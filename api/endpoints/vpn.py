...
def issue_vpn_key(payload: IssueKeyRequest) -> IssueKeyResponse:
    ...
    with connect(autocommit=False) as conn:
        ...
        activation = conn.execute(
            "UPDATE vpn_keys SET active=1 WHERE id=?",
            (record_id,),
        )
        if activation.rowcount == 0:  # pragma: no cover - defensive
            conn.rollback()
            logger.error(
                "Failed to activate VPN key after Xray sync", extra={"username": username, "uuid": uuid_value}
            )
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="failed_to_activate")
