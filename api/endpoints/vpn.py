...
from utils.xray import add_client, remove_client
...
def renew_key(payload: RenewIn, x_admin_token: str | None = Header(default=None)):
    ...
    with db.connect() as con:
        cur = con.execute(
            "SELECT * FROM vpn_keys WHERE uuid=? AND active=1", (payload.uuid,)
        )
        row = cur.fetchone()
        if not row:
        ...
        con.execute(
            "UPDATE vpn_keys SET expires_at=? WHERE uuid=?",
            (new_exp.isoformat(), payload.uuid),
        )
