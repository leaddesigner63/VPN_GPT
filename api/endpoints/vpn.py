...
async def issue_vpn_key(request: Request):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO vpn_keys (username, uuid, active, expires) VALUES (?, ?, 1, ?)",
...
async def renew_vpn_key(request: Request):
    ...
    new_exp = (
        datetime.datetime.strptime(row[0], "%Y-%m-%d") + datetime.timedelta(days=days)
    ).strftime("%Y-%m-%d")
    ...
    if os.path.exists(XRAY):
        ...
        if "inbounds" in cfg:
            ...
            cfg["inbounds"][0]["settings"]["clients"] = [
                c for c in clients if c.get("id") != uid
            ]
            json.dump(cfg, open(XRAY, "w"), indent=2)
        subprocess.run(["systemctl", "restart", "xray"], check=False)
